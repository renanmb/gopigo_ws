...
from std_msgs.msg import Bool # New code does not make use of it for motor_dir
...
class ControlsToMotors:
  def __init__(self):
   ...
   #   atexit.register(gopigo.stop)
    ...
    self.lwheel_angular_vel_motor_pub = rospy.Publisher('lwheel_angular_vel_motor', Float64, queue_size=10)
    self.rwheel_angular_vel_motor_pub = rospy.Publisher('rwheel_angular_vel_motor', Float64, queue_size=10)

    # Publish motor_cmd to the arduino ======================================
  ...
  def motorcmd_2_robot(self, wheel='left', motor_command=0):
    if self.gopigo_on:
      ...
      if wheel == 'left':
				self.motor1_cmd_pub = motor_command
        
      if wheel == 'right':
				self.motor2_cmd_pub = motor_command
  ...
  def lwheel_update(self):
    ...
    lwheel_motor_cmd = np.float64(self.angularvel_2_motorcmd(self.lwheel_angular_vel_target) )
  ...
  def rwheel_update(self):
    ...
    # Compute motor command
    rwheel_motor_cmd = np.float64(self.angularvel_2_motorcmd(self.rwheel_angular_vel_target) )
  ...
  #motor cmd publish
  def motor1_update(motor1_cmd):
    self.motor1_cmd_pub.publish(motor1_cmd)

  def motor2_update(motor2_cmd):
    self.motor2_cmd_pub.publish(motor2_cmd)
